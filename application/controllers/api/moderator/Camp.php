...
class Camp extends MY_Controller {
    ...
    {
        // модератор не должен видеть кампании админа
        $where = [
    ...
    public function get()
    {
        $camp_obj = $this->Camp2->get([
            ...
            'user_id !=' => $this->User2->get(['role' => 'administrator'])->id // hide admin camps
        ]);

        if (!$camp_obj) {
            exit_json(1, __('Кампания не существует!'));
        }
    ...
    public function add()
    {
        ...
        $this->validation->make([
            'user_id'               => "required|is_exists[users.id]|callback__is_not_admin",
    ...
    public function play()
    {
        ...
        if (!$this->Camp2->play($this->input->post_get('camp_id'), $where)) {
    ...
    public function stop()
    {
        ...
        if (!$this->Camp2->stop($this->input->post_get('camp_id'), $where)) {
            exit_json(1, __('Не удалось изменить статус кампании!'));
        }
    ...
    public function _is_not_admin($user_id)
    {
        return $user_id != $this->User2->get(['role' => 'administrator'])->id;
    }
